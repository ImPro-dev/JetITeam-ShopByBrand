...
namespace JetITeam\ShopByBrand\Model\ResourceModel;
...
class Brand extends AbstractDb
{
    ...
    {
        $this->_init('jteam_brand', 'brand_id');
    }
    ...
    public function getBrandByProduct(ProductInterface $product)
    {
        ...
        $select = $connection->select()
            ->from($this->getTable('jteam_brand_product'), 'brand_id')
    ...
    public function checkIdentifier($identifier)
    {
        $connection = $this->getConnection();
        $select = $connection->select()
            ->from($this->getTable('jteam_brand'), 'brand_id')
    ...
    public function getAssignedProductIds(BrandInterface $brand)
    {
        $connection = $this->getConnection();
        $select = $connection->select()
            ->from($this->getTable('jteam_brand_product'), 'product_id')
